...
- [x] ① 修复前后端状态/优先级映射不一致 → Status: 0=待办 2=已完成, Priority: 1=重要 2=紧急 3=普通
- [x] ② 修复任务列表 N+1 查询 → 批量查询分类名称
- [x] ③ 添加数据库索引 → tasks/users/login_log 共 5 个索引
- [x] ④ 修复用户列表返回密码哈希 → FindList 排除 password 字段
- [x] ⑤ 全量测试通过 → 40 项全部通过
- [x] ⑥ 打标签发布 → `v1.1.0`
